...
const INGREDIENT_STOCK_META = {
  flour: { unit: "kg", warning: 200, critical: 100 },
  sugar: { unit: "kg", warning: 40, critical: 20 },
  salt: { unit: "kg", warning: 10, critical: 5 },
  preservative: { unit: "g", warning: 2000, critical: 1000 },
  butter: { unit: "kg", warning: 10, critical: 5 },
  yeast: { unit: "g", warning: 500, critical: 250 },
  vegetable_oil: { unit: "L", warning: 10, critical: 5 },
  improver: { unit: "g", warning: 500, critical: 250 },
};
...
async function ensurePostgresSchema() {
  ...
  await pgPool.query(`
    ...
    CREATE TABLE IF NOT EXISTS ingredient_stock (
      ...
      unit TEXT NOT NULL,
      warning_level DOUBLE PRECISION NOT NULL DEFAULT 0,
      critical_level DOUBLE PRECISION NOT NULL DEFAULT 0,
  ...
  await pgPool.query(`
    ...
    ALTER TABLE ingredient_stock ADD COLUMN IF NOT EXISTS ingredient TEXT;
    ALTER TABLE ingredient_stock ADD COLUMN IF NOT EXISTS quantity DOUBLE PRECISION;
    ALTER TABLE ingredient_stock ADD COLUMN IF NOT EXISTS unit TEXT;
    ALTER TABLE ingredient_stock ADD COLUMN IF NOT EXISTS warning_level DOUBLE PRECISION;
    ALTER TABLE ingredient_stock ADD COLUMN IF NOT EXISTS critical_level DOUBLE PRECISION;
    ALTER TABLE ingredient_stock ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP;

    ALTER TABLE ingredient_stock_movements ADD COLUMN IF NOT EXISTS ingredient TEXT;
    ALTER TABLE ingredient_stock_movements ADD COLUMN IF NOT EXISTS change_amount DOUBLE PRECISION;
    ALTER TABLE ingredient_stock_movements ADD COLUMN IF NOT EXISTS quantity_after DOUBLE PRECISION;
    ALTER TABLE ingredient_stock_movements ADD COLUMN IF NOT EXISTS unit TEXT;
    ALTER TABLE ingredient_stock_movements ADD COLUMN IF NOT EXISTS reason TEXT;
    ALTER TABLE ingredient_stock_movements ADD COLUMN IF NOT EXISTS source_type TEXT;
    ALTER TABLE ingredient_stock_movements ADD COLUMN IF NOT EXISTS source_id INTEGER;
    ALTER TABLE ingredient_stock_movements ADD COLUMN IF NOT EXISTS actor_user_id INTEGER;
    ALTER TABLE ingredient_stock_movements ADD COLUMN IF NOT EXISTS created_at TIMESTAMP;

    CREATE UNIQUE INDEX IF NOT EXISTS idx_users_email ON users(email) WHERE email IS NOT NULL;
  `);

  const pgIngredients = Object.entries(INGREDIENT_STOCK_META);
  for (const [ingredient, meta] of pgIngredients) {
    await pgPool.query(
      `INSERT INTO ingredient_stock (ingredient, quantity, unit, warning_level, critical_level)
       VALUES ($1, 0, $2, $3, $4)
       ON CONFLICT (ingredient) DO NOTHING`,
      [ingredient, meta.unit, meta.warning, meta.critical]
    );
  }
}
...
function removeLegacySeedStaff() {
  ...
  const clearIngredientMovementActor = db.prepare("UPDATE ingredient_stock_movements SET actor_user_id = NULL WHERE actor_user_id = ?");
  ...
  const cleanup = db.transaction((users) => {
    users.forEach(({ id }) => {
      ...
      clearIngredientMovementActor.run(id);
      deleteUser.run(id);
    });
  });
...
async function initDb() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS ingredient_stock (
      ...
      unit TEXT NOT NULL,
      warning_level REAL NOT NULL DEFAULT 0,
  ...
  Object.entries(INGREDIENT_STOCK_META).forEach(([ingredient, meta]) => {
    upsertStock.run(ingredient, meta.unit, meta.warning, meta.critical);
  });
}
